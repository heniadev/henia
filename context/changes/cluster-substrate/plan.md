...
- [x] 2.5 The CRD declares `categories: [henia]` and a shortName, and `kubectl get henia` resolves — e67c16b
...
- [x] 6.1 The operator Deployment reports `Available` — e67c16b
- [x] 6.2 The `Herd` CRD is registered and `kubectl explain herd` resolves — e67c16b
- [x] 6.3 Creating a `Herd` resource succeeds and the controller logs a reconcile — e67c16b
- [x] 6.4 A deleted operator pod is replaced and reaches `Running` — e67c16b
- [x] 6.5 The operator has resource requests and limits set — e67c16b
...
- [x] 7.1 The operator appears as an `up` target in Prometheus — 671008b
- [x] 7.2 `controller_runtime_reconcile_total` is queryable and non-empty — 671008b
- [x] 7.3 Go runtime metrics for the operator process are present — 671008b
...
- [x] 8.1 The read-only identity lists `Herd` resources across namespaces — 671008b
- [x] 8.2 Creating or deleting a `Herd` as that identity is denied with 403 — 671008b
- [x] 8.3 The identity reads the operator Deployment and its pod status — 671008b
- [x] 8.4 Reading Secret contents is still denied — 671008b
